...
def process_standard_videos(limit: int = 10, logger: LoggerProtocol | None = None) -> None:
    ...
    uids = repo.get_standard_videos(limit, logger=logger)

    if not Path(WORKDIR_CM).exists():
        Path(WORKDIR_CM).mkdir(parents=True)

    for uid in uids:
        video = repo.get_video_with_segments(uid, logger=logger)
        if not video:
        ...
        with Timer(f"Traitement vidéo enhanced : {video.name}", logger):
            for seg in video.segments:
                ...
                # --- 🛠️ Remplacement
                try:
                    FileMover.safe_replace(processed_path, seg_path, logger=logger)
